...
void foo(BTree *node, int *min) {
    if (node == NULL) {
        return;
    }
    if (node->v == MASK1 + MASK2) {
        int y = node->k / SIZE;
        int x = node->k % SIZE;
        int m = abs(SIZE / 2 - y) + abs(SIZE / 2 - x);
        if (m > 0 && m < *min) {
            *min = m;
        }
    }
...
int main() {
    int x = SIZE / 2;
    int y = SIZE / 2;
    fill(line1, x, y, MASK1);
    fill(line2, x, y, MASK2);
    printf("%d\n", get_result());
}
